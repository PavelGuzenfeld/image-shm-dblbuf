...
class BlockingShmStorage
{
public:
    BlockingShmStorage(std::string const &shm_name)
        : shm_(shm::path(shm_name), sizeof(Image)),
          sem_(shm_name + "_sem", 1)
    {
    }
...
class BlockingShmDblBufLoader
{
public:
    BlockingShmDblBufLoader(std::string const &shm_name)
        : shm_(shm::path(shm_name), sizeof(Image)),
          sem_(shm_name + "_sem", 1),
          pre_allocated_(std::make_unique<Image>()),
          img_ptr_(nullptr),
    ...
    BlockingShmDblBufLoader(BlockingShmDblBufLoader const &) = delete;
    BlockingShmDblBufLoader &operator=(BlockingShmDblBufLoader const &) = delete;
    BlockingShmDblBufLoader(BlockingShmDblBufLoader &&) = delete;
    BlockingShmDblBufLoader &operator=(BlockingShmDblBufLoader &&) = delete;
...
private:
    Image *get_shm() const noexcept
    {
        return static_cast<Image *>(shm_.get());
    }
